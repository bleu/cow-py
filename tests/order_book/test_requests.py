...
async def test_no_re_attempt_if_success():
    with patch("httpx.AsyncClient.request", new_callable=AsyncMock) as mocked_request:
        ...
        result = await request(URL, path="", backoff_opts={"max_tries": 10})
    ...
    mock_call = AsyncMock(
        side_effect=[
            httpx.HTTPStatusError(
                message=ERROR_MESSAGE, request=None, response=httpx.Response(500)
            ),
            httpx.HTTPStatusError(
                message=ERROR_MESSAGE, request=None, response=httpx.Response(500)
            ),
...
async def test_succeeds_last_attempt():
    ...
    with patch("httpx.AsyncClient.request", new=mock_call):
        result = await request(URL, path="", backoff_opts={"max_tries": 4})
...
async def test_does_not_reattempt_after_max_failures():
    with patch("httpx.AsyncClient.request", new_callable=AsyncMock) as mocked_request:
        with pytest.raises(
            Exception
        ) as exc_info:  # Replace Exception with your specific exception
            mocked_request.return_value = httpx.Response(
                500, json={"error": ERROR_MESSAGE}
            )
            await request(URL, path="", backoff_opts={"max_tries": 3})
